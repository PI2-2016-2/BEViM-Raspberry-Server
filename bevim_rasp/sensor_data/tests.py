...
class DataRestTest(APITestCase):
    ...
    def setUp(self):
        ...
        self.acceleration1 = Acceleration.objects.create(sensor=self.sensor1, value=234);
        self.acceleration2 = Acceleration.objects.create(sensor=self.sensor1, value=987);
        self.speed = Speed.objects.create(sensor=self.sensor1, value=432);
        self.amplitude = Amplitude.objects.create(sensor=self.sensor1, value=123);
        self.frequency = Frequency.objects.create(sensor=self.sensor1, value=20);

    def test_get_sensor(self):
        url = reverse('sensor_rest')
        response = self.client.get(url, format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        expected_response = [{"id": self.sensor1.pk, "name": self.sensor1.name}]
